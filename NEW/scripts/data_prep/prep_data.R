...
make_iAUC <- function(metab, method, data) { 
  times.l <- list(x_30iAUC=c(0,30), x_60iAUC=c(0,30,60), 
                  x_120iAUC=c(0,30,60,120), x_180iAUC=c(0,30,60,120,180),
                  x_235iAUC=c(0,30,60,120,180,235),
                  x_270iAUC=c(235,270), x_300iAUC=c(235,270,300),
                  x_360iAUC=c(235,270,300,360))
  tag<-ifelse(method=="positive", ".pos", ifelse(method=="total", "", ifelse(method=="net", ".net", "")))
...
calc_xinsG30=function(I0, I30, G30) { return(((I30-I0) * 0.16667) / G30) }
calc_matsuda=function(xinsG30, G0, I0, G, I) {return( xinsG30*1000 / sqrt((G0*18) * I0 * 0.1667) * ((mean(G) * 18 * mean(I) * 0.1667)) )}

processed.dat <- processed.dat %>% 
  mutate(xinsG30_mmtt = calc_xinsG30(I0=insulin_0, I30=insulin_30, G30=glucose_30/18)) %>%
  mutate(xinsG30_selected = calc_xinsG30(I0=insulin_235, I30=insulin_270, G30=glucose_270/18)) %>%
  rowwise() %>%
  mutate(mean_glucose_mmtt = mean(glucose_0,glucose_30, glucose_60, glucose_120)) %>%
  mutate(mean_insulin_mmtt = mean(insulin_0,insulin_30, insulin_60, insulin_120)) %>%
  mutate(mean_glucose_selected = mean(glucose_235,glucose_270, glucose_300, glucose_360)) %>%
  mutate(mean_insulin_selected = mean(insulin_235, insulin_270, insulin_300, insulin_360, )) %>%
  ungroup() %>%
  mutate(matsuda_mmtt = calc_matsuda(xinsG30=xinsG30_mmtt, G0=glucose_0, I0=insulin_0, G=mean_glucose_mmtt, I=mean_insulin_mmtt)) %>%
  mutate(matsuda_selected = calc_matsuda(xinsG30=xinsG30_selected, G0=glucose_235, I0=insulin_235, G=mean_glucose_selected, I=mean_insulin_selected))
