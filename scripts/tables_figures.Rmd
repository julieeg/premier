...
p_glu_geno_indiv <- plot_ppXtime_stratified("glucose", times = c(0,30,60,120,180,235)) +
  ...
  annotate("text", x = 60.25, y = 140, label = "*", size = 6) +
...
p_insu_geno_indiv <- plot_ppXtime_stratified("insulin", times = c(0,30,60,120,180,235)) + 
  ...
  annotate("text", x = 60.25, y = 78, label = "*", size = 6) +
  geom_segment(aes(x = 53, xend = 67, y = 77, yend = 77), linewidth = 0.25, color="black") 
...
# Save plot as PDF
fig2ab %>% ggsave(filename="../output/fig2ab_mmtt_v2.pdf", width=6.5, height=6)
...
fig2g_formatted %>% ggsave(filename="../output/fig2g_mealxgeno_v2.pdf", height=6.5, width=2.5)
...
## Create a horizontal version for NatComms manuscript
fig2g_formatted_horiz <- fig2g+coord_flip() + 
...
fig2h %>% ggsave(filename="../output/fig2h_ssmt_v2.pdf", width=7, height=5)
...
plot_volcano.fun <- function(data, p_threshold=0.05/30, beta_threshold=0.5, title="", add_labels=T, bolded_labels=F, 
  ...
  p <- data %>% 
    ...
    theme_bw() + xlab("Beta coefficient for HC_PGS vs HF_PGS (ref) group \n of log2 fold change from 0 min") +
  ...
  if(add_labels==T) {
    p <- p + geom_label_repel(aes(label=Lab, fontface = ifelse(add_bold == "bold", "bold", "plain")), 
                              label.padding = 0.1, box.padding = 0.075, nudge_y = 0.15, #label.padding = 0.05, box.padding = 0.05, nudge_y = 0.15
                              size=3, color="black", fill=NA, label.size = 0, max.overlaps = 20)
  } ; p
}
...
tab_metab_subs %>% fwrite("../output/Tab_res_mmtt_metabolites.csv")
...
## Print summary table of genotype-stratified metabolomics results, with metabolite info (size & class)
lapply(c("HC", "HF"), function(geno) {
  res_mmtt_metab_time_bygeno %>% rename(HMDB=outcome) %>% 
    filter(genotype == paste(geno, "genotype")) %>%
    mutate(Beta.SE = sprintf("%s (%s, %s)", round(beta,1), round(lowCI,1), round(upCI, 1)), P = format_p(p)) %>%
    select(HMDB, Metabolite=Outcome, Time, Beta.SE, P) %>%
    pivot_wider(names_from="Time", values_from = c("Beta.SE", "P")) %>%
    mutate(Genotype = paste(geno, "genotype"), .before=1) %>%
    left_join(met_info_processed %>% select(Method, Compound, MZ, RT, HMDB), by = "HMDB") %>%
    left_join(metab_dict %>% rename(HMDB=HMDB_ID) %>% select(HMDB, Super.class, Main.class, Sub.class), by = "HMDB") %>%
    select(Genotype, Method, Compound, MZ, RT, Metabolite, HMDB, Super.class, Main.class, Sub.class, `Beta.SE_120 min`, `P_120 min`,
           `Beta.SE_235 min`, `P_235 min`) %>%
    arrange(`P_120 min`) %>%
    fwrite(paste0("../output/tab_res_mmtt_metab_timeEffect_byGeno_summary_", geno,"pgs.csv"))
})
```
...
bile_acids <- metab_suppl_info %>% filter(HMDB %in% all_bile_acids) %>%
  filter(P_Time_120<0.05/30 | P_Time_235<0.05/30) %>%
...
# HC genotype
bileacids.hc120 <- bileacids.all[names(bileacids.all) %in% metabs.hc120.unq]
bileacids.hc235 <- bileacids.all[names(bileacids.all) %in% metabs.hc235.unq] 
p_v.hc <- plot_volcano.fun(
  res_mmtt_metab_time_bygeno %>% 
    mutate_at("Outcome", ~gsub(" or.*", "", .)) %>%
    mutate(Outcome = ifelse(outcome %in% names(bileacids.hc120), bileacids.hc120[outcome], 
                            ifelse(outcome %in% names(bileacids.hc235), bileacids.hc235[outcome], Outcome))) %>% 
    ...
    filter(genotype=="HC genotype"), add_labels = T, bolded_labels = c(names(bileacids.hc120),names(bileacids.hc235)),
...
p_v.hf <- plot_volcano.fun(
  ...
  col1=parameters$genotype$palette[[1]], col2="#81A0C8", beta_threshold = 0.5) +
  facet_grid(cols=vars(Time)) + 
...
# Recreate volcano plots, separately by timepoint
```{r}
p_v.hcTime <- lapply(c(120, 235), function(time) {
  plot_volcano.fun(
    res_mmtt_metab_time_bygeno %>% 
      filter(Time == paste(time, "min")) %>%
      mutate_at("Outcome", ~gsub(" or.*", "", .)) %>%
      mutate(Outcome = ifelse(outcome %in% names(bileacids.hc120), bileacids.hc120[outcome], 
                              ifelse(outcome %in% names(bileacids.hc235), bileacids.hc235[outcome], Outcome))) %>% 
      mutate(Lab = ifelse(Time == "120 min" & outcome %in% metabs.hc120.unq.top20, Outcome, 
    ...
    theme(plot.margin = margin(c(0,0,-0.1,0), unit="line"),
          ...
          axis.title.y = element_markdown(size=11, margin = margin(r=-2)),
          axis.text.y = element_text(size=11),
          strip.text = element_text(face="bold", size=12)) + 
    coord_cartesian(ylim=c(0,12), xlim=c(-8,8)) + 
    scale_y_continuous(breaks=seq(0,15,4)) + 
    scale_x_continuous(breaks=seq(-8,10,4))
})

p_v.hfTime <- lapply(c(120, 235), function(time) {
  plot_volcano.fun(
    res_mmtt_metab_time_bygeno %>% mutate(beta=beta/log(2)) %>% 
      filter(Time == paste(time, "min")) %>%
      mutate_at("Outcome", ~gsub(" or.*", "", .)) %>%
    ...
    ylab("-log<sub>10</sub>(P)") +
    theme(plot.margin = margin(c(-1,0,0,0), unit="line"),
          axis.title.x = element_markdown(size=11), axis.text.x = element_text(size=11),
          axis.title.y = element_markdown(size=11, margin = margin(r=-2)),
          axis.text.y = element_text(size=11),
          strip.text = element_blank()) +
    coord_cartesian(ylim=c(0,12), xlim=c(-8,8)) +
...
fig2c.120 <- ggarrange(ggarrange(p_v.hcTime[[1]], p_v.hfTime[[1]], ncol=1, common.legend = F, legend = "none"))
fig2c.235 <- ggarrange(ggarrange(p_v.hcTime[[2]], p_v.hfTime[[2]], ncol=1, common.legend = F, legend = "none"))

# Save as wide, single column plots
fig2c.120 %>% ggsave(filename="../output/natcoms/fig3_mmtt_metab_120min.pdf", width=4.5, height=5.5)
fig2c.235 %>% ggsave(filename="../output/natcoms/fig3_mmtt_metab_235min.pdf", width=4.5, height=5.5)
```
...
# Revised pathway enrichment analysis plot, stratified by timepoint
```{r}
fig3b_time120 <- 
  metab_sets %>% 
    select(V1, log10p, Metabolite.Set, genotype, time) %>%
...
fig3b_time120 %>% ggsave(filename="../output/natcoms/fig3b_pathways_120min.pdf", height=2.5, width=6.5)
...
fig3b_time235 %>% ggsave(filename="../output/natcoms/fig3b_pathways_235min_exp.pdf", height=4.5, width=5.5)
...
fig2e <- bileacids_fc.df %>% 
  ...
  theme_bw() + ggplot_ms + 
  ylab(sprintf("120 min fold change \n(mean \u00B1 SE)")) + xlab("") + 
...
# Updated Bile acid plot, organized by primary/secondary
```{r}
## Gather bile acids to plot
bile_acids_to_plot_abbrev_type <- c("CA", "GCA", "TCA", "CDCA", "GCDCA", "TCDCA", "TUDCA", "DCA", "GDCA", "TDCA", "GLCA") # Missing LA from database
bile_acids_to_plot_type <- bile_acids$HMDB[match(bile_acids_to_plot_abbrev_type, bile_acids$Abbrev)]

# Determine order for bileacids by strength
bileacids_ordered_type <- res_mmtt_metab_time_bygeno %>% 
  filter(p<0.05/30 & beta/log(2) > 0.5) %>%
  filter(genotype == "HC genotype" & outcome %in% bile_acids_to_plot_type) %>%
  arrange(p) ; bileacids_ordered_type

bileacids_120log2fc_plot.df <- lapply(names(bileacids.all), function(b) {
  analysis2 %>% select(id, genotype, time, b) %>%
    mutate_at("time", ~paste0("m", .)) %>%
    filter(time %in% c("m0", "m120")) %>%
    pivot_wider(names_from=time, values_from=b) %>% 
    mutate(diff=m120-m0) %>% # log fold change at 120
    mutate(log2fc=diff/log(2)) %>% # log2 fold change at 120 min 
    rename_with(., ~gsub("log2fc", b, .)) %>%
    select(id, genotype, b)
  }) %>% reduce(full_join, by=c("id", "genotype"))

bileacids_235log2fc_plot.df <- lapply(names(bileacids.all), function(b) {
  analysis2 %>% select(id, genotype, time, b) %>%
    mutate_at("time", ~paste0("m", .)) %>%
    filter(time %in% c("m0", "m235")) %>%
    pivot_wider(names_from=time, values_from=b) %>% 
    mutate(diff=m235-m0) %>% # log fold change at 120
    mutate(log2fc=diff/log(2)) %>% # log2 fold change at 120 min 
    rename_with(., ~gsub("log2fc", b, .)) %>%
    select(id, genotype, b)
  }) %>% reduce(full_join, by=c("id", "genotype"))

bileacids_log2fc_plot.l <- list(m120=bileacids_120log2fc_plot.df,
                                m235=bileacids_235log2fc_plot.df)

# =================================
## Summary-level data plots (run scripts above to generate 120min FC)
## Added for R1: 1/14/2026
# =================================
ba_log2fc_bytime <- lapply(1:2, function(i) {
  bileacids_log2fc_plot.l[[i]] %>% 
    select(id, genotype, bile_acids_to_plot_type) %>%
    pivot_longer(bile_acids_to_plot_type) %>% 
    mutate_at("name", ~bileacids.all[name]) %>%
    mutate_at("genotype", ~relevel(., ref="HC genotype")) %>%
    group_by(genotype, name) %>% filter(!is.na(genotype)) %>%
    reframe(n=n(), mean=mean(value), sd=sd(value)) %>% mutate(se=sd/sqrt(n)) %>%
    mutate(type = ifelse(name %in% c("CA", "CDCA", "GCA", "TCA", "GCDCA", "TCDCA"), "Primary BA", "Secondary BA")) %>%
    mutate(time = names(bileacids_log2fc_plot.l)[i]) %>%
    mutate_at("name", ~factor(., levels=bile_acids_to_plot_abbrev_type))
}) %>% do.call(rbind.data.frame, .)

p_ba_120log2fc <- ba_log2fc_bytime %>%
  filter(type == "Primary BA") %>%
  mutate_at("time", ~factor(., levels=c("m120", "m235"), labels=c("120 min", "235 min"))) %>%
 ...
 # facet_grid(~time, scales="free_x") + 
  geom_point(size=2, position=position_dodge(0.35)) + geom_errorbar(width=0.15, linewidth=0.45, position=position_dodge(0.35)) +
  scale_color_manual(values=parameters$geno$palette, name=parameters$geno$label,
                     labels = c("HC<sub>PGS</sub>","HF<sub>PGS</sub>")) + 
  # scale_y_continuous(limits=c(0,16), breaks=c(1,5,10,15)) +
  theme_bw() + ggplot_ms + 
  ylab(sprintf("log<sub>2</sub> fold change from 0 min<br>(mean \u00B1 SE)")) + xlab("") + 
  theme(panel.background = element_blank(),
        panel.grid.minor.y = element_line(),
       strip.background = element_blank(), strip.text = element_text(face="bold", size=10),
        axis.text.x = element_text(color="black", size=10, angle=35, hjust=1, vjust=1), 
        axis.text.y = element_markdown(size=11), 
        axis.title.y = element_markdown(color="black", size=11),
        legend.direction = "horizontal", legend.title = element_blank(),
        legend.position = c(0.015, 0.9), legend.justification = c(0,0.5),
        legend.background = element_rect(fill="white", color = "black"),
        legend.text = element_markdown(size=10),
        legend.margin = margin(0.75,1.5,0.5,0.5,"mm"))

p_ba_120log2fc %>% ggsave(filename="../output/natcoms/fig3c_ba120.pdf", height=3, width=4.5)
```

# Correlations among bile acids
```{r}
# Make correlations matrix of all bile acid metabolites
cormat_ba120 <- cor(bileacids_fc.df %>% select(bile_acids_to_plot_type))
colnames(cormat_ba120) <- bileacids.all[colnames(cormat_ba120)]
rownames(cormat_ba120) <- bileacids.all[rownames(cormat_ba120)]
cormat_ba120_ordered <- cormat_ba120 %>% as.data.frame() %>% 
  mutate(order=factor(rownames(.), levels=bile_acids_to_plot_abbrev_type)) %>%
  filter(order %in% bile_acids_to_plot_abbrev_type) %>%
  arrange(order) %>%
  select(c(bile_acids_to_plot_abbrev_type)) %>%
  as.matrix()
library(ComplexHeatmap) ; library(circlize)
ComplexHeatmap::Heatmap(cormat_ba120_ordered)

# Check bile acid correlations matrix separately by PGS group
cormat_ba120_bygeno.l <- lapply(genotypes, function(g) {
  cormat_ba120_bygeno <- cor(bileacids_fc.df %>% filter(genotype == g) %>% select(bile_acids_to_plot_type))
  colnames(cormat_ba120_bygeno) <- bileacids.all[colnames(cormat_ba120_bygeno)]
  rownames(cormat_ba120_bygeno) <- bileacids.all[rownames(cormat_ba120_bygeno)]
  cormat_ba120_bygeno_ordered <- cormat_ba120_bygeno %>% as.data.frame() %>% 
    mutate(order=factor(rownames(.), levels=bile_acids_to_plot_abbrev_type)) %>%
    filter(order %in% bile_acids_to_plot_abbrev_type) %>%
    arrange(order) %>%
    select(c(bile_acids_to_plot_abbrev_type)) %>%
    as.matrix()
}) ; names(cormat_ba120_bygeno.l) <- c("hc","hf")

ComplexHeatmap::Heatmap(cormat_ba120_ordered)
hm120_hc <- Heatmap(cormat_ba120_bygeno.l$hc)  
hm120_hf <- Heatmap(cormat_ba120_bygeno.l$hf, column_order = column_order(hm120_hc))
hm120_hc+hm120_hf

## Correlations with clinical metabolites
ba_clinvars_spearman <- fread("../output/tab_res_ba_clinvars_spearman.csv")
Heatmap(ba_clinvars_spearman %>% select())
```
...
# ==============================================================
library(ComplexHeatmap) ; library(circlize)
clinvars <- c("glucose", "insulin", "tg.log", "ldl", "hdl")
clinvars.lab <- c("Glucose", "Insulin", "log(TG)", "LDL", "HDL")
names(clinvars.lab) <- clinvars

lm_ba_clin <- fread("../output/tab_res_ba_clinvars_xsect_spearman_120log2fc.csv") %>%
  ...
  filter(exposure %in% bile_acids_to_plot_abbrev_type[1:6])

hm_ba_clin <- lm_ba_clin %>% 
  arrange(p) %>%
  rowwise() %>% mutate(Outcome=factor(outcome, levels=c(clinvars), labels=c("Glucose", "Insulin","log(TG)", "LDL", "HDL"))) %>%
...
hm_palette <- colorRamp2(c(-1,0,1), c(palettes$NatExt$Greens[3], "white", palettes$NatExt$Purples[3]))
...
fig2f <- Heatmap(t(hm_plot),
        ...
        col = hm_palette,
        height = unit(4,"cm"), width=unit(4.5,"cm"),
        heatmap_legend_param = list(title=expression(phantom(0) * italic(r)),
                                    legend_title_gp = gpar(fontsize = 12),
                                    legend_text_gp = gpar(fontsize=10),
                                    title_position = "topleft")) ; fig2f

#pdf("../output/fig2f_hm_baclinvars.pdf", height=4, width=3.5) ; fig2f ; dev.off()
pdf("../output/fig3d_hm_ba120log2fc_clinvars_spearman.pdf", height=4, width=4.5) ; fig2f ; dev.off()

## REVISED
pdf("../output/natcoms/fig3d_hm_baclinvars_type.pdf", height=4, width=4.5) ; fig2f ; dev.off()
```
...
## Metabolite Lists
hchc <- res_ssmt_metab_time_bymealxgeno %>% filter(genotype == "HC genotype" & meal_type == "HC meal") %>% filter(p<0.05/23) %>% arrange(p) %>% pull(outcome)
hfhc <- res_ssmt_metab_time_bymealxgeno %>% filter(genotype == "HF genotype" & meal_type == "HC meal") %>% filter(p<0.05/23) %>% arrange(p) %>% pull(outcome)
hchf <- res_ssmt_metab_time_bymealxgeno %>% filter(genotype == "HC genotype" & meal_type == "HF meal") %>% filter(p<0.05/23) %>% arrange(p) %>% pull(outcome)
hfhf <- res_ssmt_metab_time_bymealxgeno %>% filter(genotype == "HF genotype" & meal_type == "HF meal") %>% filter(p<0.05/23) %>% arrange(p) %>% pull(outcome)
...
hchc.labs <- c(hchc.unq[1:5], names(bileacids.all)[-11]) # no change
hfhc.labs <- c(hfhc.unq[1:5], names(bileacids.all)[-11])
hchf.labs <- c(hchf.unq[1:5], names(bileacids.all)[-11])
hfhf.labs <- c(hfhf.unq[1:5], names(bileacids.all)[-11])
...
res_ssmt_metab_time_bymealxgeno <- res_ssmt_metab_time_bymealxgeno %>% 
    mutate_at("Outcome", ~case_when(
      Outcome == "Cholic acid" ~ "CA",
      Outcome == "Glycocholic acid" ~ "GCA", 
      Outcome == "Taurocholic acid" ~ "TCA", 
      Outcome == "Chenodeoxycholid acid" ~ "CDCA",
      Outcome == "Taurochenodeoxycholic acid" ~ "TCDCA", 
      Outcome == "Glycochenodeoxycholic acid" ~ "GCDCA",
      Outcome == "Glycoursodeoxycholic acid" ~ "GUDCA",
      Outcome == "Taurodeoxycholic acid" ~ "TDCA", 
      Outcome == "Glycodeoxycholic acid" ~ "GDCA",
      Outcome == "Tauroursodeoxycholic acid" | Outcome == "Taurohyodeoxycholic acid" ~ "TUDCA", 
      .default = as.character(.)))
bileacids_ordered_type

# HC genotype x HC meal
bold_ba <- names(bileacids_all)[which(names(bileacids_all) %in% hchc.unq)]
p_v.hchc <- plot_volcano.fun(
  ...
  col1="#E96900", col2="#E8AA80", beta_threshold = 0.25, p_threshold = 0.05/23) +
  ...
  scale_y_continuous(breaks=seq(0,5,1)) + scale_x_continuous(breaks=seq(-4,6,2)); p_v.hchc

# HC genotype x HF meal
bold_ba <- names(bileacids_all)[which(names(bileacids_all) %in% hchf.unq)]
p_v.hchf <- plot_volcano.fun(
  ...
  col1="#F29741", col2="#F0C398", beta_threshold = 0.25, p_threshold = 0.05/23) +
  facet_grid(cols=vars(meal_type)) + 
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "grey60", size=0.35) +
  theme(plot.margin = margin(c(-1,0,0,0), unit="line"),
        axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(),
        ...
        strip.text = element_text(face="bold", size=10)) + 
  coord_cartesian(ylim=c(0,5), xlim=c(-4,5.5)) + 
  scale_y_continuous(breaks=seq(0,5,1)) + scale_x_continuous(breaks=seq(-4,6,2)); p_v.hchf

# HF genotype x HC meal
bold_ba <- names(bileacids_all)[which(names(bileacids_all) %in% hfhc.unq)]
p_v.hfhc <- plot_volcano.fun(
  ...
  col1="#5496CE", col2="#A2BFDF", beta_threshold = 0.25, p_threshold = 0.05/23) +
  facet_grid(cols=vars(meal_type)) + 
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "grey60", size=0.35) +
  theme(plot.margin = margin(c(-1,0,0,0), unit="line"),
        axis.title.x = element_text(color="black", size=10), axis.text.x = element_text(size=10),
        axis.title.y = element_text(size=12), axis.text.y = element_text(size=10),
        strip.text = element_blank()) + 
...
bold_ba <- names(bileacids_all)[which(names(bileacids_all) %in% hfhf.unq)]
p_v.hfhf <- plot_volcano.fun(
  ...
  col1="#006EAE", col2="#81A7CB", beta_threshold = 0.25, p_threshold = 0.05/23) +
  facet_grid(cols=vars(meal_type)) + 
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "grey60", size=0.35) +
  theme(plot.margin = margin(c(-1,0,0,0), unit="line"),
        axis.title.x = element_text(color="black", size=10), axis.text.x = element_text(size=10),
