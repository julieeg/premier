...
## Load list of bile acids
ba <- readRDS("../data/processed/bileacids.rda")
bileacids_all <- ba$all
bileacids_all_abbrev <- ba$abbrev
...
### Fig 1a+b. Effect of genotype on postprandial resopnses over 4h (a) & 120 min iAUC glucose/insulin
...
p_glu_geno_indiv <- plot_ppXtime_stratified("glucose", times = c(0,30,60,120,180,235)) +
  ...
  annotate("text", x = 60.25, y = 133, label = "*", size = 6) 
p_insu_geno_indiv <- plot_ppXtime_stratified("insulin", times = c(0,30,60,120,180,235)) + 
  ...
  annotate("text", x = 60.25, y = 70, label = "*", size = 6) 
fig2a <- ggarrange("", p_glu_geno_indiv, "", p_insu_geno_indiv, common.legend = T, 
                   legend = "top", nrow=1, widths=c(0.05,1,0.05,1), #labels=c("", "a", "", ""), 
                   hjust=0.5, vjust=0.95, font.label = c(size=16), align="hv") ; fig2a 
...
fig2b <- ggarrange("", p_glu_geno_iauc, "", p_insu_geno_iauc, "", common.legend = T, 
                   legend = "none", nrow=1, widths=c(0.2,1,0.05,1,0.2), align="hv") ; fig2b
...
fig2g <- analysis %>% group_by(genotype) %>%
  ...
  scale_x_discrete(labels = parameters$genotype$levels_fancy) +
  theme_bw()+ ggplot_ms + 
  ...
  ylab("Percent") + xlab("") + 
  #coord_flip() + 
  guides(fill = guide_legend(reverse = F)) ; fig2g

fig2g_formatted <- ggarrange("", fig2g, labels = c("", "g"), widths=c(0.05,1),
          hjust=0.25, vjust=1.05,font.label = c(size=16))

fig2g_formatted %>% ggsave(filename="../output/fig2g_mealxgeno.pdf", height=6.5, width=2.5)
...
fig2h %>% ggsave(filename="../output/fig2h_ssmt.pdf", width=7, height=5.5)
...
# ================================================================
plot_volcano.fun <- function(data, p_threshold=0.05/23, beta_threshold=0.5, title="", add_labels=T, bolded_labels=F, 
                             col1, col2, colUnq=col2, p_label="0.05/23") {
...
tab_res_mmtt_metab_timeonly <- fread("../output/tab_res_mmtt_metab_timeonly.csv") 
...
tab_descr_metab_postprand_pEff <- res_mmtt_metab_timeonly %>% filter(p<0.05/23) %>% pull(outcome) %>% unique() 
length(tab_descr_metab_postprand_pEff) # N=307 metabolites changing significantly, based on correction for effective tests P <0.05/23
tab_descr_metab_postprand_pEff %>% as.data.frame() %>% fwrite("../data/processed/tab_dat_mmtt_metab_postprandial.txt")
...
tab_metab_subs <- lapply(sub_classes, function(i) {
  ...
  fwrite("../output/Tab_res_mmtt_metabolites.csv")
...
## Metabolite Lists
metabs.hc120 <- fread("../data/processed/pathways/hc120_pEff_log2fc05.txt", col.names = "V1") %>% pull(V1) ; length(metabs.hc120) #142
metabs.hc235 <- fread("../data/processed/pathways/hc235_pEff_log2fc05.txt", col.names = "V1") %>% pull(V1) ; length(metabs.hc235) #131
metabs.hf120 <- fread("../data/processed/pathways/hf120_pEff_log2fc05.txt", col.names = "V1") %>% pull(V1) ; length(metabs.hf120) #111
metabs.hf235 <- fread("../data/processed/pathways/hf235_pEff_log2fc05.txt", col.names = "V1") %>% pull(V1) ; length(metabs.hf235) #117
...
metabs.hf235[!metabs.hf235 %in% metabs.hc235] %>% length() # 27 unique metabolites for HC/120
...
p_v.all <- plot_volcano.fun(
  ...
  theme(plot.margin = margin(c(0,0,-0.1,0), unit="mm"),
        axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(),
        strip.text = element_text(face="bold", size=12),
        axis.title.y = element_text(size=12), axis.text.y = element_text(size=11)) + 
  #geom_text(data = data.frame(label="All participants (N = 17)", x=-8.35, y=20.65, size=10), 
...
p_v.hc <- plot_volcano.fun(
  res_mmtt_metab_time_bygeno %>% 
    ...
    mutate_at("Outcome", ~case_when(
      ...
      Outcome == "Tauroursodeoxycholic acid" | Outcome == "Taurohydrodeoxycholic acid" ~ "TUDCA", 
      .default = as.character(.))) %>%
      mutate(Lab = ifelse(Time == "120 min" & outcome %in% metabs.hc120.lab, Outcome, 
                          ifelse(Time == "235 min" & outcome %in% metabs.hc235.lab, Outcome, NA))) %>% 
    filter(genotype=="HC genotype"), add_labels = T, bolded_labels = names(bileacids_all),
...
p_v.hf <- plot_volcano.fun(
  res_mmtt_metab_time_bygeno %>% mutate(beta=beta/log(2)) %>% 
    mutate_at("Outcome", ~gsub(" or.*", "", .)) %>%
    mutate(Lab = ifelse(Time == "120 min" & outcome %in% metabs.hf120.lab, Outcome,
                        ifelse(Time == "235 min" & outcome %in% metabs.hf235.lab, Outcome, NA))) %>% 
...
## Vertical
fig2c <- ggarrange("", ggarrange(p_v.all, p_v.hc, p_v.hf, ncol=1, 
                                 common.legend = F, legend = "none", heights=c(1.1,0.9,1.1)), 
                   labels=c("", "c"), widths=c(0.05,1), hjust=0.25, vjust=2, font.label = c(size=16)) 
fig2c #%>% ggsave(filename="../output/fig2c_mmtt_metab.pdf", width=6, height=7)
...
metab_sets <- rbind.data.frame(
  ...
  fread("../data/metaboanalyst/hc120/v1/msea_ora_result.csv") %>% mutate(set="HC genotype @ 120 min"),
  fread("../data/metaboanalyst/hc235/v1/msea_ora_result.csv") %>% mutate(set="HC genotype @ 235 min"),
  fread("../data/metaboanalyst/hf120/v1/msea_ora_result.csv") %>% mutate(set="HF genotype @ 120 min"),
  fread("../data/metaboanalyst/hf235/v1/msea_ora_result.csv") %>% mutate(set="HF genotype @ 235 min")
  ) %>% rename(p="Raw p") %>%
...
to_plot <- c("Biosynthesis of unsaturated fatty acids", "Arachidonic acid metabolism", 
             "Fatty acid metabolism", "Primary bile acid biosynthesis",
...
bileacids_ordered <- res_mmtt_metab_time_bygeno %>% 
  filter(genotype == "HC genotype" & Outcome %in% bileacids) %>% arrange(p)
...
fig2e %>% ggsave(filename="../output/fig2e_bafoldchange.pdf", height=3, width=4.95)
```
...
lm_ba_clin <- fread("../output/tab_res_ba_clinvars_pearson.csv") %>%
...
hchc.labs <- c(hchc.unq[1:5], names(bileacids_all)[-9]) # no change
hfhc.labs <- c(hfhc.unq[1:5], names(bileacids_all)[-9])
hchf.labs <- c(hchf.unq[1:5], names(bileacids_all)[-9])
hfhf.labs <- c(hfhf.unq[1:5], names(bileacids_all)[-9])
