...
# =============================================
res_ssmt_metab_time_bymealxgeno %>%
  filter(outcome %in% bile_acids_primary$HMDB) %>%
  mutate(Beta.CI = sprintf("%s (%s, %s)", round(beta/log(2),2), round(lowCI,2), round(upCI,2))) %>%
  ...
  fwrite("../output/natcoms/tab_res_ssmt_ba_primary_genoXmeal.csv")
...
tab_metab_ssmt_bymealxgeno %>%
  ...
  mutate(gxm=paste(genotype, meal_type, sep="/")) %>%
...
tab_ssmt_ba_log2fc_bymealxgeno <- tab_metab_ssmt_bymealxgeno %>%
  filter(Exposure != "Time_(joint)") %>%
  filter(outcome %in% c(bile_acids %>% 
                          filter(Abbrev %in% bile_acids_to_plot_abbrev_type[1:6]) %>% pull(HMDB))
         ) %>%
  mutate(Beta.CI=sprintf("%s (%s, %s)", round(beta/log(2),2), round(lowCI/log(2),2), round(upCI/log(2),2)), P=format_p(p)) %>%
  mutate(gxm=paste(genotype, meal_type, sep="/")) %>%
  select(genotype, meal=meal_type, outcome, Outcome, Beta.CI, P) %>%
  pivot_wider(names_from=c(genotype), values_from=c(Beta.CI, P)) %>%
  pivot_wider(names_from=c(meal), values_from=starts_with(c("Beta.CI", "P"))) 
tab_ssmt_ba_log2fc_bymealxgeno

tab_ssmt_ba_primary_int_genoxtime <- fread("../data/processed/tab_ssmt_lme_int_ba_primary_genoxtime.csv")
